...
async def login_1():
        try:
            ...
            browser_1 = await launch(headless=True, handleSIGINT=False, handleSIGTERM=False, handleSIGHUP=False,args=['--no-sandbox', '--disable-setuid-sandbox'])
    ...
    try:
        ...
        if not browser_2 or not page_2:  # If the browser or page is not available, create them
            ...
            with open(json_file_path, 'r') as json_file:
                creds = json.load(json_file)
            # Launch the browser
            browser_2 = await launch(headless=True, handleSIGINT=False, handleSIGTERM=False, handleSIGHUP=False,args=['--no-sandbox', '--disable-setuid-sandbox'])
...
async def login_3():
    try:
        ...
        if not browser_3 or not page_3:
            browser_3 = await launch(headless=True, handleSIGINT=False, handleSIGTERM=False, handleSIGHUP=False,args=['--no-sandbox', '--disable-setuid-sandbox'])  # Set headless=True for production
        ...
        with app.app_context():  # Push the app context
            ...
            try:
                ...
                if isinstance(data_website_1, Exception):
                    ...
                    stored_data['last_time_website_1'] = datetime.now(pytz.timezone('Asia/Kolkata')).strftime('%Y-%m-%d / %H:%M')

                if isinstance(data_website_2, Exception):
                    alert_2 += "Fronius Not Updated !!"
                else:
                    stored_data['website_2'] = data_website_2
                    stored_data['last_time_website_2'] = datetime.now(pytz.timezone('Asia/Kolkata')).strftime('%Y-%m-%d / %H:%M')
                ...
                if isinstance(data_website_3, Exception):  # If website 3 raised an exception
                    ...
                    stored_data['last_time_website_3'] = datetime.now(pytz.timezone('Asia/Kolkata')).strftime('%Y-%m-%d / %H:%M')
